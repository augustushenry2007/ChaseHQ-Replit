...
const Q2 = {
  ...
  options: [
    ...
    { id: "confidence", label: "Feeling less anxious", detail: "Knowing it's handled — and I don't have to think about it" },
  ],
};

const EMPATHY_Q0: Record<string, string> = {
  anxious: "That anxiety isn't a flaw. It means you care about how you come across.",
  guilty: "That guilt is real. But you did the work — you've already earned this.",
  frustrated: "That frustration makes sense. You shouldn't have to fight this hard to get paid.",
};
...
export default function OnboardingScreen() {
  ...
  const [emotion0, setEmotion0] = useState<string | null>(null);
  const [emotion1, setEmotion1] = useState<string | null>(null);
  const [emotion2, setEmotion2] = useState<string | null>(null);
  ...
  function canAdvance() {
    if (step === 0) return !!emotion0 || !!customEmotion0;
    if (step === 1) return !!emotion1;
    if (step === 2) return !!emotion2;
  ...
  const feelingLabel = emotion0 ? Q0.options.find((o) => o.id === emotion0)?.label.toLowerCase() ?? "" : customEmotion0;
  const empathyLine = customEmotion0
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          {step === 3 && (
            <View>
              ...
              <Text style={[styles.empathyText, { color: colors.mutedForeground }]}>{empathyLine}</Text>
              ...
              {connectedIntegrations.size > 0 && (
                <View style={[styles.successBanner, { backgroundColor: "#DCFCE7", borderColor: "#86EFAC" }]}>
                  ...
                  <Text style={[styles.successText, { color: "#16A34A" }]}>
                    {connectedIntegrations.size === 1
                      ? `${Array.from(connectedIntegrations)[0]} connected`
                      : `${connectedIntegrations.size} tools connected`}
                  </Text>
                </View>
              )}
          ...
          {step === 6 && (
            <View style={{ alignItems: "center", paddingVertical: 12 }}>
              ...
              <Text style={[styles.finishTitle, { color: colors.primary }]}>
                You handle the invoicing. We handle the rest.
              </Text>
...
function QuestionStep({
  ...
  const canGo = !!selected || !!customText;
    ...
    <View>
      ...
      {setCustomText && (
        <View style={{ marginTop: 16 }}>
          <Text style={[styles.label, { color: colors.mutedForeground, fontSize: 12, marginBottom: 8 }]}>Or tell us your own reason:</Text>
          <TextInput
            value={customText}
      ...
      <TouchableOpacity
        ...
        testID="thats-me-btn"
      >
        <Text style={[styles.primaryBtnText, { color: canGo ? "#FFF" : colors.mutedForeground }]}>That's me</Text>
